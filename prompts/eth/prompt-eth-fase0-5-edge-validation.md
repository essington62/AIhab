# Prompt: ETH Phase 0.5 — Quick Edge Validation

## Contexto

Phase 0 revelou que ETH ≠ BTC clone. Descobertas principais:
- **Volume z-score vs fwd_7d: -0.387** (sinal forte!)
- **OI z-score: +0.168** em ETH (vs -0.472 em BTC — sinal invertido)
- **G9 Taker e G10 Funding: ruído** (descartar)

**Objetivo da Phase 0.5:** Validar rapidamente se esses sinais geram **edge acionável** antes de investir em HMM/scoring complexo.

**Filosofia:** simplicidade cirúrgica. Sem HMM, sem scoring elaborado. Apenas **análise condicional de edge isolado**.

## Hipóteses a validar

### H1 — Volume z-score é preditivo
```
volume_z < -1 → fwd return POSITIVO maior que baseline?
volume_z > +1 → fwd return NEGATIVO maior que baseline?
```

### H2 — OI z-score tem edge (sinal invertido)
```
oi_z > +1 → fwd return POSITIVO maior que baseline? (ETH bullish)
oi_z < -1 → fwd return NEGATIVO maior que baseline?
```

### H3 — Combinação volume + OI amplifica
```
volume_z < -1 AND oi_z > +1 → edge combinado supera individuais?
```

### H4 — Sinais são estruturais (não artefato de janela)
```
Correlações consistentes em [90d, 180d, 270d, 365d]?
```

## Estrutura do script

### Criar `scripts/eth_phase0_5_edge_validation.py`
...
def load_data():
    ...
    for col in df.columns:
        if "oi" in col.lower() or "open_interest" in col.lower():
            # Skip if it's our own derived column
            if col in ["oi_z"]:
                continue
            oi_col = col
            break
...
def run_tests(df):
    ...
    # Generate summary
    # ------------------------------------------------------
    generate_summary(vol_corr, oi_corr if "oi_z" in df.columns else None, bt_results, baseline)
...
1. [ ] Criar `scripts/eth_phase0_5_edge_validation.py` (código acima)
2. [ ] Rodar: `python scripts/eth_phase0_5_edge_validation.py`
3. [ ] Ler output no terminal (análises + backtest)
4. [ ] Verificar `prompts/eth_phase0_5_summary.md`
5. [ ] Decidir próximo passo baseado nos critérios do summary
6. [ ] Git commit + push

## Critério de decisão (copiado para clareza)

```
PROCEED PHASE 1 (implementar ETH trading) IF:
  ✅ Volume corr significant em 3+ windows (estrutural)
  ✅ Backtest WR > 55%
  ✅ Backtest Sharpe > 1.0
  ✅ Backtest max DD < 15%

CONTINUE RESEARCH IF:
  ⚠️ Alguns sinais fortes mas não todos critérios

ABANDON ETH IF:
  ❌ WR < 45%
  ❌ Correlações mudam sinal entre windows
...
Se a descoberta de volume_z -0.387 for estrutural e acionável, você tem um sistema ETH em 2-3 semanas usando apenas essa feature + OI invertido. **Não precisa de HMM, scoring de 11 gates, adaptive weights.**

Se não for, economizou meses tentando forçar ETH no framework BTC.

## Próximos passos baseados em resultado

| Resultado | Próxima ação |
|-----------|--------------|
| PROCEED | Implementar estratégia simples ETH (Phase 1 — 2-3 semanas) |
| CONTINUE | Adicionar macro/stablecoin ao estudo, re-testar |
| ABANDON | Focar 100% em BTC, considerar ETH só se outra oportunidade surgir |
